...
- ⚠⚠ **`tags.hasFloorPlan:false` + `domains.medias.floorplans:[]` can BOTH lie — the Grundrisse then
  ...
  **any image without a `classification` key gets fetched and Read, on every feed.**
  ...
  selector would have reported "no unit plan" on #785. **#786 (H1-03-15) + #787 (H5-00-05) + #788 (H5-01-11) + #789 (H6-00-01) + #790 (H6-00-05) = `floorplans[0]` again → 3 images-tail / 6 floorplans.** Also sum the plan on EVERY unit: #789's advertised m² was copied from a Haus-5 sister (plan ≠ ad), #790's matched exactly — correctness is per unit, not per house.
