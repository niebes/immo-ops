...
- **invisible-playwright can HANG (not just crash) — on Immowelt, go to CiC FIRST.** `new_page` returns nothing for the full 1800s idle timeout: seen 2026-07-20, **recurred 2026-08-09 (#538)**, i.e. twice = the norm, not a blip. Symptom differs from the "Connection closed while reading from the driver" crash below but the remedy is the same: don't retry, use CiC. *Why:* each attempt burns 30 min of wall-clock; the standing "PREFER invisible-playwright" doctrine is a net loss on this portal specifically. Immowelt renders fully in CiC with no consent wall, so the fallback is cheap.
  - CiC entry sequence that works: `tabs_context_mcp{createIfEmpty:true}` (a bare `tabs_create_mcp` errors with "No tab group exists for this session yet") → `navigate` → `javascript_tool`. The 2026-07 "first navigate lands on chrome://newtab" no-op did NOT recur on 2026-08-09; one navigate loaded the expose.
...
- **Check `Bausubstanz und Energie` in `innerText` BEFORE opening the energy modal.** When there is no certificate at all the block spells it out in plain text — #538: "Energieausweis / **Der Energieausweis wird bei Besichtigung nachgereicht.** / Baujahr 1995 / Zustand der Immobilie Gepflegt / Heizungsart / Energieträger". That is a complete answer (no class, no kWh) and a § 87 GEG violation worth flagging; opening the modal would waste 2 calls. Only escalate to the modal when the block shows a class/value teaser or a bare "Mehr Infos" button.
