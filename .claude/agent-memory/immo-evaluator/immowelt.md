...
- ⚠⚠ **`tags.hasFloorPlan:false` + `domains.medias.floorplans:[]` can BOTH lie — the Grundrisse then
  ...
  **any image without a `classification` key gets fetched and Read, on every feed.**
  ...
  selector would have reported "no unit plan" on #785. **#786 (H1-03-15) + #787 (H5-00-05) + #788 (H5-01-11) + #789 (H6-00-01) + #790 (H6-00-05) = `floorplans[0]` again → 3 images-tail / 6 floorplans.** **#791 (H6-01-06) = images-tail again (Bild 59/60) → 4 / 6.** **#792 (H6-01-11) + #793 (H6-02-12) + #826 (H4-01-03) = `floorplans[0]` → 4 / 9.**
...
- **All load-bearing fields are in `innerText`** — extract by section, not one blob (CiC truncates ~1100 chars):
  ...
  - **KAUF listings — `Preisdetails` is a ready-made Block A.** Gives Kaufpreis, €/m², `Provision für Käufer` (%), and a full itemized `Kaufnebenkosten` + **`Geschätzte Gesamtkosten`** (Notar 1,5 % / Grunderwerbsteuer / Provision / Grundbuch 0,5 %). ⚠ **The breakdown can DEGRADE to a single bogus line — count the items before trusting the total.** #809 (`b52adfa1-…`, Grundstück Marquardt) itemized `Kaufnebenkosten` as ONLY „Provision für Käufer (**19 %**)" = 39.275 € — which is 19 % *of the Kaufpreis*, i.e. the portal printed the **MwSt rate as the commission rate** — while GrESt, Notar and Grundbuch were missing outright. The real commission per `price.base.commissionFee` was „3,3 % zuzüglich 19 % MwSt." = 3,927 % = **8.117,50 €**, so the advertised „Geschätzte Gesamtkosten 245.985 €" was wrong in both directions at once (true all-in 232.397,85 €; +16,2 % over a 200-T-EUR cap rather than the +23 % the portal implies). ⇒ Whenever `price.breakdown.groups[].items` has fewer than the usual four entries, recompute by hand (Kaufpreis + GrESt 6,5 % BB + Notar ~1,5 % + Grundbuch ~0,5 % + Provision) and never quote Immowelt's total. Same record also had `tags.hasBrokerageFee: false` contradicting the stated Provision — check the two against each other, and on a *Bauunternehmen* selling its own land ask whether the „Provision" is really Kaufpreis (⇒ grunderwerbsteuerpflichtig) and whether a Bauverpflichtung makes it an einheitliches Vertragswerk (GrESt on plot + build). Immediately after it, `Preise in der Region` states whether the €/m² is above/below comparable regional objects, plus the regional min/max €/m². *Why:* no need to hand-compute Nebenkosten or WebSearch a market benchmark — but note Immowelt's Grunderwerbsteuer line is the **state** rate (Brandenburg 6,5 %), so verify it matches the property's state, and the "günstiger als vergleichbare" verdict is an AVIV estimate over a very wide band, so treat it as weak evidence only.
...
- **Grundstück (PLOT) exposés have a SHORTER `sections` set — `energy` is absent outright, so a
  ...
  ⚠ **…but those two Erschließung chips are NOT guaranteed — `features.preview` can carry ONLY
  `availability` + `plotSpace`, and then a prose „voll erschlossen" has zero structured backing.**
  #809 (`b52adfa1-…`, Marquardt): `features.preview` = exactly 2 chips (`availability: "Bezug: Nach
  Vereinbarung"`, `plotSpace: "504 m² Grundstück"`), `features.details: null` — no
  `site-development-state`, no `development-infrastructure`, while the description asserted „Das ebene
  und **voll erschlossene** Grundstück". *Why:* the #762 note reads as „read the chips instead of the
  prose"; when the chips are absent the reflex is to take the prose as equivalent. It is not — score
  Erschließung as **claimed-but-unverified** (Block D) and put § 133 Abs. 3 BauGB beitragsfrei? in Next
  Steps. Same page: a full keyword sweep for `Bebauungsplan|B-Plan|§ 34|Innenbereich|Bauvoranfrage|
  Baugenehmigung|Flurstück` returned **0 hits across the whole 607-KB payload** ⇒ Baurecht at the
  bottom rung of the `_shared.md` ladder („nothing but the lister's claim"). Cheap tell worth keeping:
  on an unbebautes Grundstück a `medias.images[].classification.name` of **`HOUSE_FACADE`** cannot be
  the object (neighbour or reference house) — #809 had 3 images = 2× `YARD` + 1× `HOUSE_FACADE`, i.e.
  effectively 2 usable pictures and no Lageplan.
