...
- ⚠⚠ **`tags.hasFloorPlan:false` + `domains.medias.floorplans:[]` can BOTH lie — the Grundrisse then
  ...
  **any image without a `classification` key gets fetched and Read, on every feed.**
  ...
  selector would have reported "no unit plan" on #785. **#786 (H1-03-15) + #787 (H5-00-05) + #788 (H5-01-11) + #789 (H6-00-01) + #790 (H6-00-05) = `floorplans[0]` again → 3 images-tail / 6 floorplans.** **#791 (H6-01-06) = images-tail again (Bild 59/60) → 4 / 6.** **#792 (H6-01-11) = `floorplans[0]` → 4 / 7.** On this lister the headline's „Balkon mit {West|Ost}ausrichtung" matched the plan's north arrow 2/2 (#791 Ost, #792 West), so it is a cheap first read. Still check the plan, because the WE number does NOT predict orientation across Häuser (WE 06 is West in Haus 5 but East in Haus 6). Also sum the plan on EVERY unit: #789's advertised m² was copied from a Haus-5 sister (plan ≠ ad), #790's matched exactly — correctness is per unit, not per house.
