...
- **A portal's `city`/`district` field is not evidence of location — resolve in this order: prose → `zipCode` → street signs legible in the gallery photos → nothing else.** Immowelt's `city`/`district` has been wrong 8+ times (Berlin swaps labelled Grunewald that were Charlottenburg 10627 and Schöneberg 10779; a Havelland plot labelled Potsdam that was Paaren im Glien 14621, ~19 km out). A `geometry` polygon does **not** falsify it — both are derived from the same lister-chosen geo id, so the polygon just confirms the error. Getting this wrong misscores Block B and can silently put an out-of-area object in the results.
- **Lister-declared unavailability beats every liveness test.** A lister often marks a flat as gone by editing the **title** rather than deleting the ad — the page stays HTTP 200 with a complete payload and even `isNew: true`, so the usual liveness checks pass on a dead flat. Sweep **every** headline-bearing field on the first call — on Immowelt that is `sections.mainDescription.headline` AND `classified.title` AND `document.title`, because on the Tauschwohnung-GmbH feed `classified.title` holds the full description while the real headline lives only in `mainDescription.headline`. Match `vergeben|bereits vergeben|nicht mehr verfügbar|reserviert|VERMIETET|VERKAUFT` and mark **EXPIRED** on a hit; corroborate with `updateDate`. (Seen 2026-09-12 on #712: "TAUSCHWOHNUNG VERGEBEN: 3-Zimmerwohnung", edited the same morning.)
...
- **Never test the ADVERTISED Warmmiete against the `max_warmmiete` cap.** A listed "Warmmiete" routinely means Kaltmiete + Nebenkosten with **Heizkosten omitted**; reconstruct the real warm figure (add ~1,00–1,40 EUR/m²/month for heating) before comparing. Seen 2026-09-12 across the Brauhausberg charge: #732's advertised 2.175,00 EUR sat 25 EUR *under* the 2.200 cap and the real figure was ~2.260–2.295, i.e. 60–95 EUR *over*. Where a batch of similar units is close to the cap, this reconstruction — not EUR/m² — is what actually discriminates between them.
...
- **Prefer a plain `curl` over any browser when the portal has a data route.** ImmoScout24 answers fully on `api.mobile.immobilienscout24.de/expose/{id}` with UA `ImmoScout24_1410_35_._`; Kleinanzeigen detail pages render server-side; and Regionalimmobilien24 returns the full server-rendered detail page to a plain `curl` with a Firefox UA (stable since late August 2026) — none of the three needs a browser at all. This is the single biggest lever on evaluation cost, and a curl-only evaluation holds no browser lock, so it is safe to run in parallel with other evaluations (see `immo-find` auto Step 4).
